...
export default function Navbar({ theme = 'light' }) {
    ...
    return (
        <nav className={`w-full px-4 md:px-12 z-50 fixed top-0 left-0 right-0 transition-colors duration-300 ${bgColor}`}>
            <div className={`w-full py-6 flex justify-between items-center border-b ${borderColor} relative z-50`}>
                <Link href="/" className={`text-lg font-medium ${textColor}`} onClick={() => setIsOpen(false)}>
                    Yavuz Dağdelen
                </Link>

                {/* Desktop Navigation */}
                <div className="hidden md:flex space-x-8">
                    <Link href="/work" className={`text-base hover:opacity-70 transition-opacity ${textColor}`}>
                        Work
                    </Link>
                    <Link href="/about" className={`text-base hover:opacity-70 transition-opacity ${textColor}`}>
                        About
                    </Link>
                    <Link href="/blog" className={`text-base hover:opacity-70 transition-opacity ${textColor}`}>
                        Blog
                    </Link>
                </div>
