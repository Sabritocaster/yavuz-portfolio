...
export default function ProjectCard({ project, className = "" }) {
    const CardContent = () => (
        <>
            ...
            <h3 className={`text-sm mb-1 font-extrabold transition-opacity ${!project.wip ? 'group-hover:opacity-70' : 'opacity-70'}`}>{project.title}</h3>
            <p className={`text-sm font-extrabold ${project.wip ? 'text-gray-500' : 'text-gray-500'}`}>{project.description}</p>
        </>
    );
